classdef DataTable < handle
    ...
    methods
        ...
        %-----------------------------------------
        %Method 7: Transform DataTable to a matrix
        %-----------------------------------------
        function matrx = DataTableToMatrix(obj)
		...
		%-----------------------------------------
        %Method 8: Transform Matrix to a DataTable
        %-----------------------------------------
		function MatrixToDataTable(obj,matrx)
            matRows = size(matrx,1);
			matCols = size(matrx,2);
			obj = DataTable(matCols);
            for i = 1:1:matRows
				row = Row(matrx(i,1:matCols));
				obj.AddRow(row)
            end
        end
