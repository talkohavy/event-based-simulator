classdef GraphList < handle
    ...
    methods
        ...
        %-----------------  CONSTRUCTOR  ---------------------
        %-----------------  CONSTRUCTOR  ---------------------
        function obj=GraphList(matrx,names)
            %----------------------------------
            ...
            %STEP 3: Read Graph
            %------------------
            obj.adjacentList = MyList.empty(0);
            for i = obj.vertexes:-1:1
                obj.adjacentList(i) = MyList();
            end
            for i = 1:1:obj.vertexes
                for j = 1:1:obj.vertexes
                    if (matrx(i,j) == 1)
                        entity = Entity(j,NaN);
                        obj.adjacentList(i).Enque(entity);
                    end
                end
            end
            %------------------
        ...
        function BFS(obj,s)
            ...
            while (queue.size > 0)
                ...
                %--------------------------------
                iNod = obj.adjacentList(curVertex).GetFirst();
                for j = 1:1:obj.adjacentList(curVertex).size
                    curNeighbor = iNod.GetEntity().data;
                    %---------------------------------------
        ...
        function DfsVisit(obj,cur)
            ...
            obj.time = obj.time + 1;
            iNod = obj.adjacentList(cur).GetFirst();
            for j = 1:1:obj.adjacentList(cur).size
                curNeighbor = iNod.GetEntity().data;
                if (obj.infoMap(obj.color, curNeighbor) == 0)%White
                    obj.paths(curNeighbor) = cur;
                    obj.DfsVisit(curNeighbor);
                else
                    if (obj.infoMap(obj.color, curNeighbor) == 1)%Gray
                        obj.isDag = false;
                    end
                end
                iNod = iNod.GetNext();
        ...
        function obj = TopologicalSort(obj)
            if (obj.DfsRan)
                ...
                obj.topologySorted = zeros(1,obj.vertexes);
                for i = 1:1:obj.vertexes
