classdef Simulation < handle
    ...
    methods
        ...
        function RunOnce(obj)
            ...
            dataArr = [obj.Tnow,obj.rpPoolArr(2).size,obj.rpPoolArr(3).size,obj.rpPoolArr(4).size,obj.rpPoolArr(5).size,obj.rpInAssemblyArr(2),obj.rpInAssemblyArr(3),obj.rpInAssemblyArr(4),obj.rpInAssemblyArr(5),obj.DL2.size,obj.Us,obj.repNumber];
        ...
        function CreateP_Faster(obj,curU)
            ...
            while (1)
                ...
                addedTime = addedTime + obj.TimeToCreateP(obj.pArr(1),obj.pArr(2));
                %curU.data = obj.Tnow + x; %data = eventTime 
        ...
        function CreateRP(obj,curU,addedTime)
            %Step 1: Time to Create.
            x = addedTime + obj.TimeToCreateRP(obj.rpArr(1),obj.rpArr(2));
            curU.data = obj.Tnow + x; %data = eventTime 
            %Step 2: Update Entity's eventCode.
            ...
            %---------
            %Policy 2: Batch Production (Specific Model)
            %---------
%             if (obj.batchProd == 1)
%                 u = rand();
%                 if (u < 0.5)
...
%             end
            ...
            %---------
%             minim = 2;
%             obj.which = obj.graphMatrix.vertexes;
%             maxim = obj.rpNeededArr(obj.which);
%             for i = obj.graphMatrix.vertexes:-1:minim
...
%             end
        ...
        function CreateSelfAssembly(obj,partialU,curRP)
            ...
            x = obj.TimeToSelfAssemble(obj.saArr(1),obj.saArr(2));
...
%                 end
                ...
                %********* Changes with structure *********
                %------------------------------------------
                if (curRP.ID == 2) %P1 in the Serial Model
...
%                     obj.rpInAssemblyArr(r1) = obj.rpInAssemblyArr(r1) + 1;
                    ...
                    %Free P1: (Move P1 from stillForced to notForced)
                ...
                if (obj.graphMatrix.adjacentMat(curRP.ID, othrRP) == 1)
                    ...
                    if (partialU.dynamicArray(othrRP) <= 0)%Actually is it ==, but this catches errors
                        ...
                        if (obj.rpPoolArr(othrRP).size > 0)
                            ...
                            %Next we need to select 1 RP from RP pool:
