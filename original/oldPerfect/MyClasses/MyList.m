        ...
        function InsertionSort(obj)
            ...
            if (isnan(jNod) == 0 && isnan(jNod.GetNext) == 0)
                ...
                while (isnan(jNod.GetNext) == 0)
                    ...
                    key = jNod.Clone();
                    iNod = jNod.GetPrev;
                    if (iNod.GetEntity.data > key.GetEntity.data)
                        ...
                        if (broke == true)
                            ...
                            key.SetNext(obj.first);
                            key.SetPrev(NaN);
                            obj.first.SetPrev(key);
                            obj.SetFirst(key);
                        end
                    end
                    jNod = jNod.GetNext;
                ...
                if(iNod.GetEntity.data > jNod.GetEntity.data)
                    ...
                    obj.last = jNod.GetPrev;%obj.SetLast(jNod.GetPrev);
                    obj.last.SetNext(NaN);%obj.GetLast.SetNext(NaN);
                    %Step 6: Go Backwards.
                    iNod = iNod.GetPrev;
                    while(isnan(iNod) == 0)
                        if (iNod.GetEntity.data < jNod.GetEntity.data)
                    ...
                    if (broke == true)
                        ...
                        jNod.SetNext(obj.first);%obj.GetFirst);
                        jNod.SetPrev(NaN);
                        obj.first.SetPrev(jNod);%obj.GetFirst.SetPrev(jNod);
                        obj.first= jNod;%obj.SetFirst(jNod);
                    end
                end
            end
        end
